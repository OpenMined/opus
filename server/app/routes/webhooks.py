...
from app.utils.permissions import secure_webhook
...
def webhooks_issue_credential():
    ...
    user = Users.query \
        .join(SessionState, SessionState.user_id == Users.user_id) \
    ...
    try:
        ...
        SessionState.query.find_by(connection_id=connection_id).first() \
            .update(state="credential-error")

    else:
        SessionState.query.find_by(connection_id=connection_id).first() \
            .update(state="credential-issued")
...
def handle_connection_response(message):
    ...
    try:
        ...
        current_app.logger.exception("Error sending credential offer:")
        SessionState.query.find_by(connection_id=str(message["connection_id"])).first() \
