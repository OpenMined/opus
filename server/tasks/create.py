...
@task(help={
    "username": "The username to which the client will be attached",
...
def client(ctx, username='test', client_name="Awesome Client",
    ...
    with app.app_context():
        ...
        user = Users.query.filter_by(username=username).first()
        if not user:
            print(f"User with username {username} could not be found")
